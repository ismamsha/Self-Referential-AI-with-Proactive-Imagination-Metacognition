"""
DSM Benchmark B — Developmental Self Model
==========================================
...
Lifetime learning
-----------------
Synapses change by a three-factor rule: local eligibility (pre/post activity,
collected over each instruction->response trial) x a neuromodulator driven by
reward-prediction-error. Output synapses use post = onehot(action) - policy.
Actions are sampled from softmax(logits / T) with an evolved temperature.
The initial genome already contains a working version of this rule, so ES
refines a learner instead of searching for one from nothing.

Curriculum (recommended)
------------------------
Stage 1 - easy: 1 task, 3 cues, 3 actions, no faults, fixed size.
    python dsm_benchmark_b.py --preset easy --generations 100 --population 64 --lives 32 --checkpoint dsm_b_easy.pt

Stage 2 - more cues/actions, development on:
    python dsm_benchmark_b.py --preset easy --n-cues 6 --n-actions 6 --development on --complexity-cost 0.005 --generations 150 --lives 32

Stage 3 - full Benchmark B:
    python dsm_benchmark_b.py --preset full --steps 480 --generations 400 --population 128 --lives 32

Quick CUDA smoke test
---------------------
    python dsm_benchmark_b.py --preset easy --generations 2 --population 8 --lives 4 --steps 80
...
class Config:
    ...
    n_cues: int = 6                  # cue identities per task
    n_actions: int = 6               # response choices
    fault_probability: float = 0.65
    damage_scale: float = 1.0        # multiplies damage from wrong answers
    cue_gain: float = 2.0            # amplitude of task/cue one-hot inputs
    ...
    max_structural_change: int = 3
    development: bool = True         # False = fixed network size (no grow/prune)
    ...
    # Fitness
    complexity_cost: float = 0.020
    structural_change_cost: float = 0.002
    survival_bonus: float = 5.0
    death_penalty: float = 5.0
    accuracy_weight: float = 0.0     # fitness += weight * lifetime accuracy (0..1)
...
class Scenarios:
    ...
    def __init__(self, cfg: Config, n: int, seed: int, device: torch.device):
        ...
        # Common random numbers for stochastic action sampling (exploration).
...
class BenchmarkB:
    ...
    def _trial_task(self, trial: int, phase: int) -> torch.Tensor:
        n_active_tasks = phase + 1
        return torch.clamp(
            (self.task_u[:, trial] * n_active_tasks).long(),
            min=0,
            max=n_active_tasks - 1,
        )
    ...
    def observation(self) -> Tuple[torch.Tensor, bool, int]:
        ...
        if is_instruction:
            ...
            # Amplified so the cue is not drowned out by energy/bias/time.
    ...
    def step(
        ...
        if is_instruction:
            ...
            cost = 0.006 + 0.006 * action_scale

            # One actuator becomes unexpectedly expensive after a hidden fault.
            expensive = motor_fault & (action == self.fault_action)
            cost = torch.where(expensive, cost * 4.0, cost)

            energy = torch.where(
                live, torch.clamp(energy - cost, min=0.0), energy
            )

            # Wrong decisions slowly damage the body.
            damage_add = self.cfg.damage_scale * torch.where(
                fragile_fault,
                torch.full_like(damage, 0.035),
                torch.full_like(damage, 0.010),
            )
            damage = torch.where(
            ...
            damage = torch.where(
                correct, torch.clamp(damage - 0.003, min=0.0), damage
            )
...
def genome_spec(cfg: Config, obs_dim: int) -> List[Tuple[str, Tuple[int, ...]]]:
    ...
    return [
        ...
        # Neuromodulator sees:
        # doubt(3), |doubt|(3), reward-prediction-error, learnability, bias = 9
        ("modulator", (9,)),

        # Log policy temperature for stochastic action sampling.
        ("temperature", (1,)),
...
def initial_genome(
    ...
    for name, shape in genome_spec(cfg, obs_dim):
        if name == "rule_out":
            # Start as a working three-factor rule:
            #   dW_out = RPE * hidden * (onehot(action) - policy)
            # i.e. A=1, B=C=D~0. Evolution refines it from here.
            value = 0.03 * rng.standard_normal(shape)
            value[0, :] += 1.0
            ...
            # Input/recurrent start slow; output starts fast enough to
            # learn within a lifetime (0.15 * sigmoid(1.0) ~= 0.11).
            value = np.array([-2.5, -2.5, 1.0])

        elif name == "modulator":
            # Positive dependence on reward-prediction-error (index 6),
            # no constant bias -> no blind Hebbian drift.
            value = np.zeros(shape)
            value[6] = 2.0

        elif name == "temperature":
            value = np.array([math.log(0.3)])
            ...
            value = np.zeros(shape)
            # Bias initial behavior strongly toward HOLD.
            value[0, -1] = 2.0
            value[1, -1] = -1.0
            value[2, -1] = -1.0
...
def run_lives(
    ...
    # Eligibility traces for input/recurrent synapses. They accumulate the
    # local rule over one instruction->response trial and are consolidated
    # only when the response outcome (reward) is known.
    elig_in = torch.zeros_like(birth_in)
    elig_rec = torch.zeros_like(birth_rec)

    # Running reward baseline -> reward-prediction-error for the modulator.
    ...
    recent_accuracy = torch.full((batch,), 1.0 / A, device=device)
    ...
    for t in range(cfg.steps):
        ...
        # Birth recurrent weights are drawn with 1/sqrt(max_hidden). Only a
        # few units are active, so rescale by sqrt(H / active) to keep the
        # effective recurrent gain ~1 and preserve the cue across the delay.
        ...
        hidden = torch.tanh(
            ...
            + rec_gain[:, None]
            * torch.einsum("bi,bij->bj", previous_hidden, birth_rec)
        ...
        # Stochastic policy (exploration) with common random numbers.
        temperature = torch.exp(
            torch.clamp(g["temperature"][:, 0], -4.0, 2.0)
        )
        policy = torch.softmax(logits / temperature[:, None], dim=1)
        action = (
            (policy.cumsum(dim=1) < env.action_u[:, t, None]).sum(dim=1)
        ).clamp(max=A - 1)
        ...
        # Error EMA / learning progress are tracked on response steps only.
        # Mixing instruction steps (near-zero error) with response steps made
        # "progress" flip sign every step, so learnability only learned the
        # instruction/response rhythm.
        ...
        if not is_instruction:
            ...
            # This is intentionally the pre-update prediction: what the
            # organism believed about its learning ability at this moment.
        ...
        #
        # Three-factor learning: local eligibility (pre/post activity)
        # x neuromodulator. Eligibility is collected over the whole
        # instruction->response trial; synapses only change on the response
        # step, when the outcome is known. The modulator is driven by a
        # reward-prediction-error (reward - running baseline), so correct
        # answers strengthen and wrong answers weaken what was just done.
        # ---------------------------------------------------------
        ...
        if is_instruction:
            ...
            rpe = (task_reward - reward_baseline) * lived.float()
            reward_baseline = torch.where(
                lived,
                0.9 * reward_baseline + 0.1 * task_reward,
                reward_baseline,
            )
            ...
            # Output post-synaptic term: chosen action minus policy
            # (local, REINFORCE-like; no backprop through the network).
        ...
        prune_mask = (structural_action == 2) & lived

        if not cfg.development:
            # Fixed-size network: always HOLD.
            grow_mask = torch.zeros_like(grow_mask)
...
def evolve(cfg: Config, device: torch.device) -> torch.Tensor:
    ...
    print(
        ...
        f"{cfg.n_cues} cues, {cfg.n_actions} actions, delayed response"
    )
    print(f"Chance accuracy    : {100.0 / cfg.n_actions:.1f}%")
    print(
        f"Faults / damage    : p={cfg.fault_probability}, "
        f"damage_scale={cfg.damage_scale}"
    )
    print(
        f"Development        : {'on' if cfg.development else 'off (fixed size)'}"
        f", complexity_cost={cfg.complexity_cost}"
        f", accuracy_weight={cfg.accuracy_weight}"
    )
    print(
    ...
    for generation in range(cfg.generations + 1):
        ...
        if generation % 10 == 0:
            # ES health: if std is tiny relative to per-life noise, ES has no
            # signal -> raise --lives.
            life_noise = result["fitness"].std(dim=1).mean()
            print(
                f"  ES pop fitness: mean {fitness.mean().item():7.2f}"
                f" | std {fitness.std().item():6.3f}"
                f" | best {fitness.max().item():7.2f}"
                f" | per-life noise {life_noise.item():6.2f}"
            )
...
# Presets. Any flag passed explicitly on the command line overrides these.
#   easy : curriculum stage 1 - get lifetime learning above chance first.
#   full : the original Benchmark B difficulty.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="difficulty preset; explicit flags override it",
    )
    ...
    # Preset-controlled (default None = take value from preset).
    p.add_argument("--steps", type=int, default=None)
    p.add_argument("--max-tasks", type=int, default=None)
    p.add_argument("--n-cues", type=int, default=None)
    p.add_argument("--n-actions", type=int, default=None)
    p.add_argument("--fault-probability", type=float, default=None)
    p.add_argument("--damage-scale", type=float, default=None)
    p.add_argument(
        "--development", type=lambda s: s.lower() in ("1", "true", "yes", "on"),
        default=None, help="on/off: allow grow/prune",
    )
    p.add_argument("--complexity-cost", type=float, default=None)
    p.add_argument("--accuracy-weight", type=float, default=None)
